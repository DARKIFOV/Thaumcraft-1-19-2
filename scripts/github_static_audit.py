...
def png_audit() -> list[str]:
    ...
    for path in (ROOT / "src/main/resources").rglob("*.png"):
        ...
        is_original_bank = ("/textures/original/" in rel or "/original_tc4_1710/" in rel or "/textures/item/tc4/" in rel or "/textures/block/tc4/" in rel)
...
def stage115_config_aspects_runtime_audit() -> list[str]:
    errors: list[str] = []
    required = [
        "STAGE115_VALIDATION_REPORT.json",
        "docs/porting/STAGE115_TC4_CONFIG_ASPECTS_RUNTIME_PORT.md",
    ...
    for rel in required:
        if not (ROOT / rel).exists():
            errors.append(f"missing Stage115 ConfigAspects runtime file: {rel}")
    aspect_db = ROOT / "src/main/java/com/darkifov/thaumcraft/AspectDatabase.java"
    text = aspect_db.read_text(encoding="utf-8", errors="ignore") if aspect_db.exists() else ""
    for token in ["TC4ObjectAspectRegistry.getAspectsForItem", "TC4ObjectAspectRegistry.getAspectsForBlock"]:
        if token not in text:
            errors.append(f"AspectDatabase missing Stage115 token: {token}")
    registry = ROOT / "src/main/java/com/darkifov/thaumcraft/source/TC4ObjectAspectRegistry.java"
    rtext = registry.read_text(encoding="utf-8", errors="ignore") if registry.exists() else ""
    for token in ["legacyOreDictionaryEntries", "exactRuntimeEntries", "thaumcraft:thaumonomicon", "minecraft:iron_ingot"]:
        if token not in rtext:
            errors.append(f"TC4ObjectAspectRegistry missing token: {token}")
    return errors
...
def main() -> None:
    checks = {
        ...
        "Stage115 ConfigAspects runtime port": stage115_config_aspects_runtime_audit(),
    }

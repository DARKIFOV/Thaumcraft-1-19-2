...
checks=[
 ...
 ['python3','tools/java_parse_guard_116419.py'],
 ['python3','tools/java_syntax_guard.py'],
 ['python3','tools/validate_json_resources.py'],
 ['python3','tools/validate_runtime_manifest.py','--manifest','runtime_artifacts/runtime_test_manifest.template.json','--version','11.64.19','--template'],
...
for i,c in enumerate(checks,1):
 ...
 if r.returncode:
  print(f'FOCUSED STATIC CI v11.64.19: FAIL at check {i}',file=sys.stderr); raise SystemExit(r.returncode)
print(f'FOCUSED STATIC CI v11.64.19: PASS ({len(checks)}/{len(checks)})')

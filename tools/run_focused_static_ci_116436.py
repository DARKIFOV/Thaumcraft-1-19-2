...
checks = [['python3', 'tools/tc4_116436_essentia_tube_full_closure_guard.py'],
 ...
 ['python3', 'tools/validate_runtime_manifest.py', '--manifest', 'runtime_artifacts/runtime_test_manifest.template.json', '--version', '11.64.36', '--template'],
...
for i,cmd in enumerate(checks,1):
    ...
    try:
        ...
        print(f"FOCUSED STATIC CI v11.64.36: TIMEOUT at {i}",file=sys.stderr,flush=True)
        raise SystemExit(124)
    print(p.stdout,end='' if p.stdout.endswith('\n') else '\n',flush=True)
    if p.returncode:
        print(f"FOCUSED STATIC CI v11.64.36: FAIL at {i}",file=sys.stderr,flush=True)
        raise SystemExit(p.returncode)
print(f"FOCUSED STATIC CI v11.64.36: PASS ({len(checks)}/{len(checks)})")

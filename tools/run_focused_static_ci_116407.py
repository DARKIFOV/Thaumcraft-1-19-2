...
checks = [
 ...
 ['python3', 'tools/validate_runtime_manifest.py', '--manifest', 'runtime_artifacts/runtime_test_manifest.template.json', '--version', '11.64.07', '--template'],
...
for i, c in enumerate(checks, 1):
    ...
    if x.returncode:
        print(f'FOCUSED STATIC CI v11.64.07: FAIL at check {i}', file=sys.stderr)
        raise SystemExit(x.returncode)
print(f'FOCUSED STATIC CI v11.64.07: PASS ({len(checks)}/{len(checks)})')

...
checks = [
    ...
    ['python3', 'tools/java_parse_guard_116412.py'],
    ...
    ['python3', 'tools/validate_runtime_manifest.py', '--manifest',
     'runtime_artifacts/runtime_test_manifest.template.json', '--version', '11.64.12', '--template'],
...
for index, command in enumerate(checks, 1):
    ...
    if result.returncode:
        print(f'FOCUSED STATIC CI v11.64.12: FAIL at check {index}', file=sys.stderr)
        raise SystemExit(result.returncode)
print(f'FOCUSED STATIC CI v11.64.12: PASS ({len(checks)}/{len(checks)})')

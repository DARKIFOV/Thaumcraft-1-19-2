#!/usr/bin/env python3
"""Static parity guard for the TC4 4.2.3.5 Thaumic Slime port in v11.62.92."""
...
need(".github/workflows/build.yml", "Validate v11.62.92 Thaumic Slime parity")
need(".github/workflows/release.yml", "Validate v11.62.92 Thaumic Slime parity")
forbid(entity, "Math.min(16")

if errors:
    print(f"TC4 11.62.92 Thaumic Slime parity guard: FAIL ({len(errors)} problems; {checks} checks)")
    for error in errors:
        print(" -", error)
    sys.exit(1)
print(f"TC4 11.62.92 Thaumic Slime parity guard: PASS ({checks}/{checks} checks)")

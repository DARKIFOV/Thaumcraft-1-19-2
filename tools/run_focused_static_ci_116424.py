...
checks=[
 ...
 ['python3','tools/java_parse_guard_116424.py'],
 ['python3','tools/java_syntax_guard.py'],
 ['python3','tools/validate_json_resources.py'],
 ['python3','tools/validate_runtime_manifest.py','--manifest','runtime_artifacts/runtime_test_manifest.template.json','--version','11.64.24','--template'],
...
with ThreadPoolExecutor(max_workers=8) as pool:
 ...
 for future in as_completed(futures):
  i,c,code,out=future.result()
  results[i]=(c,code,out)
...
for i in range(1,len(checks)+1):
 ...
 if code:
  print(f'FOCUSED STATIC CI v11.64.24: FAIL at check {i}',file=sys.stderr)
  raise SystemExit(code)
print(f'FOCUSED STATIC CI v11.64.24: PASS ({len(checks)}/{len(checks)})')

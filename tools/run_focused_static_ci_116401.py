...
checks=[
 ...
 ['python','tools/java_parse_guard_116401.py'],
 ['python','tools/validate_json_resources.py'],
 ['python','tools/validate_runtime_manifest.py','--manifest','runtime_artifacts/runtime_test_manifest.template.json','--version','11.64.01','--template'],
...
for i,cmd in enumerate(checks,1):
 ...
 if r.returncode:
  print(f'FOCUSED STATIC CI v11.64.01: FAIL at check {i}',file=sys.stderr); raise SystemExit(r.returncode)
print(f'FOCUSED STATIC CI v11.64.01: PASS ({len(checks)}/{len(checks)})')

#!/usr/bin/env python3
"""Regression guard for v11.62.91 compile-risk and runtime evidence contracts."""
...
for rel, tokens in {
    ...
    "runtime_artifacts/runtime_test_manifest.template.json": ('"version": "11.62.91"', '"outer_lands"', '"tests"'),
    ".github/workflows/build.yml": ("Validate runtime evidence manifest template", "11.62.91", "gradle-build.log", "continue-on-error: true", "Fail job after preserving compiler log"),
    ".github/workflows/release.yml": ("Validate runtime evidence manifest template", "11.62.91", "gradle-build.log", "continue-on-error: true", "Fail job after preserving compiler log"),
}.items():
    for token in tokens:
        need(rel, token)

if errors:
    print(f"TC4 11.62.91 compile/runtime contract guard: FAIL ({len(errors)} problems; {checks} checks)")
    for error in errors:
        print(" -", error)
    sys.exit(1)
print(f"TC4 11.62.91 compile/runtime contract guard: PASS ({checks} checks)")

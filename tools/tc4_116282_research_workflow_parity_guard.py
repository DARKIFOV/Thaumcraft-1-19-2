#!/usr/bin/env python3
"""Static regression guard for v11.62.88 TC4 research-note/table workflow parity."""
...
require("build version 11.62.88", "version = '11.62.88'" in build)
require("mods version 11.62.88", 'version="11.62.88"' in mods)
...
require("runtime manifest version 11.62.88", manifest.get("version") == "11.62.88")
...
if errors:
    print(f"TC4 11.62.88 research workflow parity guard: FAIL ({len(errors)} problems; {checks} checks)")
    for problem in errors:
        print(f" - {problem}")
    raise SystemExit(1)

print(f"TC4 11.62.88 research workflow parity guard: PASS ({checks}/{checks} checks)")

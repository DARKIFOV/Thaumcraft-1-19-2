#!/usr/bin/env python3
"""Static regression guard for v11.62.89 TC4 research-note/table workflow parity."""
...
require("build version 11.62.89", "version = '11.62.89'" in build)
require("mods version 11.62.89", 'version="11.62.89"' in mods)
...
require("runtime manifest version 11.62.89", manifest.get("version") == "11.62.89")
...
if errors:
    print(f"TC4 11.62.89 research workflow parity guard: FAIL ({len(errors)} problems; {checks} checks)")
    for problem in errors:
        print(f" - {problem}")
    raise SystemExit(1)

print(f"TC4 11.62.89 research workflow parity guard: PASS ({checks}/{checks} checks)")

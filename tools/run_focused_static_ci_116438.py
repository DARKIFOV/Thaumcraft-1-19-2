...
checks = [['python3', 'tools/tc4_116438_alchemical_centrifuge_full_closure_guard.py'],
...
for i,cmd in enumerate(checks,1):
    ...
    try:
        ...
        print(f"FOCUSED STATIC CI v11.64.38: TIMEOUT at {i}",file=sys.stderr,flush=True)
        raise SystemExit(124)
    print(p.stdout,end='' if p.stdout.endswith('\n') else '\n',flush=True)
    if p.returncode:
        print(f"FOCUSED STATIC CI v11.64.38: FAIL at {i}",file=sys.stderr,flush=True)
        raise SystemExit(p.returncode)
print(f"FOCUSED STATIC CI v11.64.38: PASS ({len(checks)}/{len(checks)})")

...
checks=[
 ...
 ['python','tools/java_parse_guard_116399.py'],
 ['python','tools/validate_json_resources.py'],
 ['python','tools/validate_runtime_manifest.py','--manifest','runtime_artifacts/runtime_test_manifest.template.json','--version','11.63.99','--template'],
...
for i,cmd in enumerate(checks,1):
    ...
    if result.returncode:
        print(f'FOCUSED STATIC CI v11.63.99: FAIL at check {i}',file=sys.stderr)
        raise SystemExit(result.returncode)
print(f'FOCUSED STATIC CI v11.63.99: PASS ({len(checks)}/{len(checks)})')

...
for lang in ["en_us","ru_ru"]:
    data=json.loads((ROOT/f"src/main/resources/assets/thaumcraft/lang/{lang}.json").read_text())
    checks += 2
...
if errors:
    print(f"TC4 11.62.90 Brain Jar runtime guard: FAIL ({len(errors)} problems)")
    for e in errors: print(" -",e)
    sys.exit(1)
print(f"TC4 11.62.90 Brain Jar runtime guard: PASS ({checks} checks)")

...
req('version agreement',"version = '11.64.22'" in b and 'version="11.64.22"' in m)
...
run('Arcane Lamp full-closure guard',['python3','tools/tc4_116422_arcane_lamp_full_closure_guard.py'])
run('Java 17 self-test and targeted parse',['python3','tools/java_parse_guard_116422.py'])
run('JSON resources',['python3','tools/validate_json_resources.py'])
run('runtime manifest',['python3','tools/validate_runtime_manifest.py','--manifest','runtime_artifacts/runtime_test_manifest.template.json','--version','11.64.22','--template'])
...
req('GameTest uniqueness',len(methods)==190 and len(methods)==len(set(methods)),str(len(methods)))
man=json.loads((R/'runtime_artifacts/runtime_test_manifest.template.json').read_text()); ids=[x['id'] for x in man['tests']]
req('manifest uniqueness',man['version']=='11.64.22' and len(ids)==606 and len(ids)==len(set(ids)),f"{man['version']}/{len(ids)}")
forbidden=[]
for p in R.rglob('*'):
 rel=p.relative_to(R)
...
report=R/'TC4_11.64.22_ARCANE_LAMP_FULL_CLOSURE_PARITY_PORT_REPORT_RU.md'
ev=R/'tools/data/tc4_arcane_lamp_full_source_evidence_v11.64.22.json'; root_ev=R/'TC4_11.64.22_ARCANE_LAMP_SOURCE_EVIDENCE.json'
...
log=(R/'TC4_11.64.22_GRADLE_BUILD_ATTEMPT.log').read_text(errors='replace'); code=(R/'TC4_11.64.22_GRADLE_BUILD_EXITCODE.txt').read_text().strip()
req('honest Gradle evidence','UnknownHostException: services.gradle.org' in log and 'BUILD SUCCESSFUL' not in log and code=='1')
ci=(R/'TC4_11.64.22_FOCUSED_STATIC_CI_FINAL.log').read_text(errors='replace')
req('focused CI evidence','FOCUSED STATIC CI v11.64.22: PASS (32/32' in ci)
print(f'FINAL RECHECK v11.64.22: PASS ({len(passed)}/{len(passed)})')

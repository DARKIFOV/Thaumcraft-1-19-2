#!/usr/bin/env python3
"""Regression guard for the 11.62.84 client initialization crash fixed in v11.62.91."""
...
need("runtime_artifacts/runtime_test_manifest.template.json", '"version": "11.62.91"')
need("tools/validate_runtime_manifest.py", 'default="11.62.91"')

if errors:
    print(f"TC4 11.62.91 runtime init hotfix guard: FAIL ({len(errors)} problems; {checks} checks)")
    for error in errors:
        print(" -", error)
    sys.exit(1)

print(f"TC4 11.62.91 runtime init hotfix guard: PASS ({checks}/{checks} checks)")

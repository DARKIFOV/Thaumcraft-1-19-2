...
checks = [
    ...
    ['python3', 'tools/java_parse_guard_116415.py'],
    ...
    ['python3', 'tools/validate_runtime_manifest.py', '--manifest',
     'runtime_artifacts/runtime_test_manifest.template.json', '--version', '11.64.15', '--template'],
...
for index, command in enumerate(checks, 1):
    ...
    if result.returncode:
        print(f'FOCUSED STATIC CI v11.64.15: FAIL at check {index}', file=sys.stderr)
        raise SystemExit(result.returncode)
print(f'FOCUSED STATIC CI v11.64.15: PASS ({len(checks)}/{len(checks)})')

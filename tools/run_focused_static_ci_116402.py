...
checks=[
 ['python','tools/tc4_116402_research_completion_warp_guard.py'],
 ['python','tools/research_completion_warp_selftest.py'],
 ['python','tools/tc4_116401_research_note_completion_guard.py'],
 ['python','tools/research_note_completion_selftest.py'],
 ...
 ['python','tools/java_parse_guard_116402.py'],
 ['python','tools/validate_json_resources.py'],
 ['python','tools/validate_runtime_manifest.py','--manifest','runtime_artifacts/runtime_test_manifest.template.json','--version','11.64.02','--template'],
...
for i,cmd in enumerate(checks,1):
 ...
 if r.returncode:
  print(f'FOCUSED STATIC CI v11.64.02: FAIL at check {i}',file=sys.stderr); raise SystemExit(r.returncode)
print(f'FOCUSED STATIC CI v11.64.02: PASS ({len(checks)}/{len(checks)})')

...
with tempfile.TemporaryDirectory() as td:
    ...
    if result.returncode:
        print('Targeted Java parse guard v11.64.12: FAIL (pure contract)')
        print(result.stdout)
        raise SystemExit(1)
...
for path in FORGE:
    with tempfile.TemporaryDirectory() as out:
        result = subprocess.run(
            ['javac', '--release', '17', '-proc:none', '-d', out,
             '-XDshould-stop.at=PARSE', '-Xmaxerrs', '500', str(path)],
    ...
    if bad:
        print(f'Targeted Java parse guard v11.64.12: FAIL ({path.name})')
        print('\n'.join(bad[:150]))
        raise SystemExit(1)

print('Targeted Java parse guard v11.64.12: PASS (3 changed Java files; pure contract compiled with --release 17)')

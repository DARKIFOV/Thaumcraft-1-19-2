#!/usr/bin/env python3
"""Regression guard for v11.63.10 wand capacity, complete subtype catalogue, JEI and BEWLR parity."""
...
check("all craftable sceptres exposed", "CREATIVE_VARIANT_COUNT = WAND_AND_STAFF_VARIANT_COUNT * 2" in variants
      and "variants.add(create(rod, cap, true, true))" in variants)
...
check("legacy ModelBox UV and face normals", "legacyModelQuad(" in renderer
      and "Matrix3f normal = poseStack.last().normal()" in renderer
      and ".normal(normalMatrix, normalX, normalY, normalZ)" in renderer)
...
print(f"TC4 11.63.10 wand variants/vis/JEI/texture guard: {len(checks)-len(failed)}/{len(checks)} PASS")
if texture_failures:

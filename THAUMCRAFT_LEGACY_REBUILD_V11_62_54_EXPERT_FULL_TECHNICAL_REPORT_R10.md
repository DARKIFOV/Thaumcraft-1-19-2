# Thaumcraft Legacy Rebuild v11.62.54-hotfix9 — полный экспертный технический отчёт, редакция R10

**Снимок кода:** v11.62.54  
**Исправленный пакет:** v11.62.54-hotfix9  
...
| 11.62.54-hotfix9 | Исправлена генерация zero-page исследований `CAP_iron` и `ROD_wood`; устранены две подтверждённые ошибки Java-компиляции. |
...
# Приложение P. Hotfix9 — исправление Java-компиляции после анализа logs_79010398282

## P.1. Подтверждённая причина падения

Новый GitHub Actions job успешно прошёл все 32 статические проверки, включая регенерационную сверку 201 исследований и полный аудит Таумономикона. Сбой произошёл только на `:compileJava`:

```text
TC4OriginalResearchPageIndex.java:108: error: ')' expected
TC4OriginalResearchPageIndex.java:123: error: ')' expected
```

В сгенерированном Java находились две незавершённые конструкции:
...
`CAP_iron` и `ROD_wood` в оригинальном TC4 являются виртуальными auto-unlock маркерами компонентов жезла и не имеют собственных `ResearchPage`. Генератор корректно обрабатывал исследования с одной или несколькими страницами, но для пустого массива страниц оставлял открытый вызов `put(...)`.

## P.2. Реализованное исправление

- исправлен `tools/generate_tc4_research_runtime_from_source_map.py`;
- исследования без страниц теперь генерируются как корректные zero-vararg вызовы:
...
- заново сгенерирован `TC4OriginalResearchPageIndex.java`;
- в генератор добавлена обязательная самопроверка обоих zero-page ключей;
- `java_syntax_guard.py` расширен проверкой баланса `()`, `[]` и `{}` вне строк и комментариев;
- build/release workflows, `build.gradle`, `mods.toml`, README и аудит переведены на `11.62.54-hotfix9`;
- подтверждённая ошибка исправлена в источнике генерации, а не только вручную в производном Java-файле.

## P.3. Выполненные проверки

После исправления успешно выполнены все статические проверки workflow, включая:

- 1699 JSON-файлов;
- 940/940 byte-exact оригинальных текстур;
- 690 item-моделей, 0 missing/unresolved;
- 8/8 BEWLR-контрактов;
- 201 исследование, 591 страница, 272 recipe references;
- 0 отсутствующих иконок и языковых ключей;
- deterministic generation check — PASS.

Дополнительно четыре автономных Java-класса research index были скомпилированы напрямую `javac --release 17`, включая `TC4OriginalResearchPageIndex.java`. Ошибки `')' expected` больше не воспроизводятся.

## P.4. Доказательная граница

Логи GitHub Actions доказали, что ForgeGradle, Forge 43.5.2 и Java 17 успешно дошли до настоящей Java-компиляции. Hotfix9 устраняет обе показанные компилятором синтаксические ошибки. В локальной изолированной среде повторная ForgeGradle-сборка может зависеть от доступа к `services.gradle.org`; окончательный статус полного JAR должен подтверждаться следующим запуском GitHub Actions.

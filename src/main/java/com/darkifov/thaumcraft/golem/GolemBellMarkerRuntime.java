...
/**
 * Stage199 adapter for original TC4 ItemGolemBell marker semantics.
 * Original NBT keys intentionally use the 1.7.10 names: golemid, golemhomex/y/z,
 * golemhomeface and markers[{x,y,z,dim,side,color}].
 */
public final class GolemBellMarkerRuntime {
    ...
    public static ThaumGolemEntity boundGolem(ItemStack stack, Level level) {
        ...
        return entity instanceof ThaumGolemEntity golem ? golem : null;
    }

    public static ToggleResult changeMarkers(ItemStack stack, Player player, Level level, BlockPos pos, Direction side, boolean markMultipleColors) {
        ...
        if (!markMultipleColors) {
            index = indexOf(markers, pos, dim, sideByte, (byte) -1, true);
        } else {
            for (byte color = -1; color < 16; color++) {
                index = indexOf(markers, pos, dim, sideByte, color, false);
                foundColor = color;
                if (index >= 0) {
                    break;
                }
        ...
        if (index >= 0) {
            ...
            if (markMultipleColors && !player.isShiftKeyDown()) {
                ...
                if (next <= 15) {
                    markers.add(new Marker(pos.getX(), pos.getY(), pos.getZ(), dim, sideByte, next));
                    action = next < 0 ? "any" : "color " + next;
                }
            }
        } else {
            markers.add(new Marker(pos.getX(), pos.getY(), pos.getZ(), dim, sideByte, (byte) -1));
            action = "added";
        }
    ...
    public static ListTag markerListFromTaskPositions(BlockPos home, BlockPos input, BlockPos output, BlockPos guard, BlockPos work, int homeFacing, Level level) {
        List<Marker> markers = new ArrayList<>();
        int dim = dimensionId(level);
        addIfPresent(markers, work, dim, (byte) homeFacing, (byte) -1);
        addIfPresent(markers, input, dim, (byte) homeFacing, (byte) 0);
        addIfPresent(markers, output, dim, (byte) homeFacing, (byte) 1);
        addIfPresent(markers, guard, dim, (byte) homeFacing, (byte) 2);
        addIfPresent(markers, home, dim, (byte) homeFacing, (byte) 3);
    ...
    public static MutableComponent markerSummary(ItemStack stack) {
        ...
        return Component.literal("TC4 bell id=" + getGolemId(stack) + " home=" + homeText + " markers=" + markers.size());
    }

    private static void addIfPresent(List<Marker> markers, BlockPos pos, int dim, byte side, byte color) {
        if (pos != null && !pos.equals(BlockPos.ZERO)) {
            markers.add(new Marker(pos.getX(), pos.getY(), pos.getZ(), dim, side, color));
        }
    }

    private static int indexOf(List<Marker> markers, BlockPos pos, int dim, byte side, byte color, boolean ignoreColor) {
        for (int i = 0; i < markers.size(); i++) {
            Marker marker = markers.get(i);
            if (marker.x() == pos.getX() && marker.y() == pos.getY() && marker.z() == pos.getZ()
                    && marker.dim() == dim && marker.side() == side
    ...
    private static int dimensionId(Level level) {
        if (level == null) {
            return 0;
        }
    ...
    public record Marker(int x, int y, int z, int dim, byte side, byte color) {
    }
